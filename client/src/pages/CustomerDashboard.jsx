      ...
      <div className="space-y-12 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Hello, {user.name}! Track your inquiries and manage your service experience.</p>
          </div>
          <Link to="/services" className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition shadow-sm flex items-center gap-2">
             🔍 Browse New Services
          </Link>
        </div>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <StatCard icon="🔧" label="Services Used" value={loading ? '—' : distinctServices.length} color="sky" />
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <p className="text-xs text-slate-400 font-medium">1 Inquiry per Service</p>
          </div>
        ...
        <section>
          ...
          {distinctServices.length === 0 ? (
             <p className="text-sm text-slate-400 italic">No services contacted yet.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
               {distinctServices.slice(0, 8).map((svc) => (
                 <div key={svc._id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
                    <div className="absolute top-0 right-0 p-3 flex flex-col items-end gap-1">
                       <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${
                          svc.bookingStatus === 'completed' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                          svc.type === 'booked' ? 'bg-sky-50 text-sky-700 border border-sky-100' : 'bg-slate-50 text-slate-500 border border-slate-100'
                       }`}>
                          {svc.bookingStatus || svc.type}
                       </span>
                    </div>

                    <div className="w-12 h-12 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center text-xl mb-4 group-hover:scale-105 transition-transform">
                       {svc.bookingStatus === 'completed' ? '✅' : '🔧'}
                    </div>
                    <h3 className="font-bold text-slate-900 mb-1">{svc.title}</h3>
                    <p className="text-xs text-slate-400 mb-6">{svc.category?.name || 'Home Service'}</p>
                    
                    <div className="flex flex-col gap-2 relative z-10">
                       {svc.type === 'booked' && svc.bookingStatus !== 'completed' && (
                          <button 
                            onClick={() => handleComplete(svc.bookingId)}
                            className="w-full py-2.5 bg-brand-600 text-white rounded-xl text-xs font-bold hover:bg-brand-700 transition"
                          >
                             🏁 Mark Completed
                          </button>
                       )}
                       {svc.type === 'contacted' && svc.messageStatus === 'accepted' && !svc.bookingId && (
                          <Link 
                            to={`/billing/${svc._id}`}
                            className="w-full py-2.5 bg-slate-900 text-white rounded-xl text-xs font-bold text-center hover:bg-slate-800 transition shadow-sm"
                          >
                             💳 Proceed to Pay
                          </Link>
                       )}
                       <Link to={`/services/${svc._id}`} className="block text-center py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition">
                          View Details
                       </Link>
                    </div>
                 </div>
        ...
        <section>
          <div className="bg-slate-800 rounded-3xl p-8 shadow-sm">
            <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-3">
              <span className="w-10 h-10 bg-slate-700 rounded-xl flex items-center justify-center text-lg shadow-sm border border-slate-600">⭐</span>
              Rate Your Recent Experiences
            </h2>
            <p className="text-slate-300 text-sm mb-8">Help others by sharing your thoughts on services you've contacted recently.</p>
            
            {pendingReviewServices.length === 0 ? (
               <div className="bg-slate-700/50 backdrop-blur-sm rounded-2xl p-6 text-center border border-slate-700">
                  <p className="text-sm font-bold text-slate-300">You're all caught up! No pending reviews.</p>
               </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {pendingReviewServices.map(svc => (
                   <div key={svc._id} className="bg-slate-700/80 rounded-2xl p-5 shadow-sm border border-slate-600 flex items-center justify-between hover:bg-slate-700 transition">
                      <div>
                        <p className="font-bold text-white text-sm">{svc.title}</p>
                        <p className="text-xs text-amber-400 mt-0.5">Awaiting your feedback</p>
                      </div>
                      <Link to={`/feedback/${svc._id}`} className="px-4 py-2 bg-amber-500 text-slate-900 rounded-xl text-xs font-bold hover:bg-amber-400 transition shadow-sm">
                         Write Review
                      </Link>
                   </div>
                 ))}
